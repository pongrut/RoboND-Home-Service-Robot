...
int main (int argc, char **argv)
{
  ...
  while (ros::ok ())
      ...
      while (marker_pub.getNumSubscribers () < 1)
	{
      ...
      // Looping util the robot reach pick up location
      while (!pickup_status)
	{
	  // Call chatterCallback to read robot position
	  ros::spinOnce ();
          // Read rosparam to check the pick up status
	  ros::param::get ("/pick_objects/pickup_success", pickup_status);
	  sleep (0.1);
	}

      // Check whether the object's picked up
      if (pickup_status)
	{
     ...
     // Looping util the robot reach drop off location
      while (!delivery_status)
	{
          // Call chatterCallback to read robot position
	  ros::spinOnce ();
          // Read rosparam to check the delivery status
	  ros::param::get ("/pick_objects/delivery_success", delivery_status);
	  sleep (0.1);
	}
      
      // Check whether the object's delivered
      if (delivery_status)
	{
